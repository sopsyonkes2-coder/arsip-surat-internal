...
export default function ArsipPage() {
  ...
  const years = useMemo(() => {
    const uniqueYears = Array.from(
      new Set(
        archives
          .map((archive) => getYear(archive.tanggalDiterima))
          .filter(Boolean)
      )
  ...
  const filteredArchives = useMemo(() => {
    ...
    return archives.filter((archive) => {
      const matchesSearch =
        !searchValue ||
        archive.nomor.toLowerCase().includes(searchValue) ||
        archive.nomorSurat.toLowerCase().includes(searchValue) ||
        archive.nomorAgenda.toLowerCase().includes(searchValue) ||
        archive.pengirim.toLowerCase().includes(searchValue) ||
        archive.perihal.toLowerCase().includes(searchValue);

      const matchesYear =
        year === "Semua Tahun" ||
        getYear(archive.tanggalDiterima) === year;

      const matchesMonth =
        month === "Semua Bulan" ||
        getMonth(archive.tanggalDiterima) === month;

      const matchesClassification =
        classification === "Semua Klasifikasi" ||
        archive.klasifikasi === classification;
  ...
  }, [archives, search, year, month, classification]);
  ...
  const resetFilters = () => {
    ...
    setClassification("Semua Klasifikasi");
  };

  const deleteArchive = async (archive: ArchiveData) => {
    if (
      !window.confirm(
        `Hapus arsip nomor ${archive.nomor}? Tindakan ini tidak dapat dibatalkan.`
      )
    ) {
      return;
    }
    ...
    try {
      const response = await fetch(
        `/api/archives?id=${encodeURIComponent(archive.nomor)}`,
        { method: "DELETE" }
      );
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.message || "Gagal menghapus arsip.");
      }
      await loadArchives();
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-sm font-medium text-blue-600">Arsip</p>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      ...
                      <p className="mt-1 text-xs text-slate-400">
                        Agenda: {archive.nomorAgenda}
                      </p>
                    </td>

                    <td className="whitespace-nowrap px-5 py-4 text-slate-600">
                      {formatDate(archive.tanggalSurat)}
                    </td>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  {canDelete && (
                    <button
                      type="button"
                      onClick={() => deleteArchive(archive)}
                      className="flex items-center justify-center rounded-xl bg-red-50 px-4 py-2.5 text-red-600"
                      title="Hapus arsip"
                    >
                      <Trash2 size={16} />
                    </button>
